/**
 * Barberia.mx — UI hooks (Editorial)
 * Versión Astro: masthead/header/footer son server-rendered; este script
 * solo aplica los hooks post-render (sin fetch/inject). Antes vivía en components.js.
 */
(function () {
  'use strict';
  ...
  function hookMobileNav() {
    ...
    document.querySelectorAll('.nav-mobile-link').forEach((link) => { link.addEventListener('click', close); });
  ...
  const AUTO_REVEAL_SELECTORS = [
    '.ed-section-head', '.ed-section-intro', '.ed-feature', '.ed-product', '.ed-job',
    '.ed-blog-card', '.ed-manifesto-inner', '.ed-state-card', '.ed-feat-story',
    '.ed-dept', '.ed-article-row', '.ed-atlas-tile',
  ];
  ...
  function run() {
    hookCurrentYear(); hookMastheadDate(); hookMastheadIssue();
    hookActiveNav(); hookMobileNav(); hookNavbarScroll();
    hookSmoothScroll(); hookDropdowns(); hookAutoReveal(); hookReveal();
